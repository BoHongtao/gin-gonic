...
## 1、说出linux中的常用的15个指令 ##
 ls cp rm mv cd init su pwd touch mkdir useradd usermod userdel groupadd groupmod groupdel
目录切换指令ls cd pwd
 init 
用户切换su
文件操作：touch  cp  mv  rm
文件夹操作：mkdir  cp -r  mv  rm -rf
用户操作：useradd usermod userdel 
组操作：groupadd groupmod groupdel
权限：Chmod
## 2、说说使用nginx遇到过的问题 ##
nginx是不支持tp框架的PATH_INFO模式的，需要配置重写规则
请求的访问路径中含有 .php 时, 通过正则表达式构造出 PATHINFO, 并设置到 fastcgi 的参数 PATH_INFO 中
## 3、什么是负载均衡？如何处理高并发的请求？有什么解决方案？ ##
当一台web服务器无法满足并发请求时，可以通过增加web服务器来提高性能，这时就需要使用到负载均衡技术。负载均衡技术就是负责把用户的请求分发到不同的web服务器。

## 4、用过memcache吗？你的项目中的哪些数据被存入了memcache？为什么存入？ ##
...
## 6、为什么把session数据存入memcache？ ##
简单的来说是为了让各台服务器的session数据共享

## 7、如何把memcache的缓存时间设为30天？ memcahce缓存的最大有效期是多少？ ##
缓存周期有两种设置方式：
...
## 10、myisam和innodb区别 ##
数据的文件存储格式：
Innodb的数据文件由两个文件构成：结构，索引和数据，默认情况下，所有表的数据和索引文件存一个文件中，需要手动配置使每个表拥有独立的数据索引文件
Myisam的数据文件：数据-索引-结构 存在不同的文件
数据的记录存储顺序：
Innodb(不适合大量数据的插入操作)，因为会根据主键进行排序，影响插入性能
Myisam(适合大量数据的插入)，数据的存储顺序就是插入顺序，没有排序
    并发性：
Innodb:支持行锁和行锁，并发性好，支持外键，事务
Myisam：只支持表锁，并发性差,支持压缩（压缩后不能插入新数据，要先解压，一般来说这个表只需要查询操作就压缩这个表）
总结：
myisam： 插入数据非常快，适合使用场合dedecms/phpcms/discuz/微博系统等插入、读取操作多的系统。
innodb: 适合业务逻辑比较强的系统，修改 操作较多的，例如ecshop、crm、办公系统、商城系统
## 11、实现数据库的优化方法 ##
选择合适的存储引擎（根据业务需求）
逆范式/反三范式：故意违反三范式，增加冗余字段，减少连表查询，提高查询效率。
通过profile查看每条语句执行时间并优化sql语句（加索引）
查询缓存：提高查询效率。
分区技术：单表数据条数比较大，例如1亿条记录。
分表技术：解决表中大数据的问题
数据碎片修复：数据表的update和delete操作比较频繁，周期性的进行数据碎片修复
...
## 13、一千万条记录，进行分页展示，20条记录为一页，会有什么问题？ ##
页码越大，查询效率越低。
				解决：① 业务，禁止分页大于100页。
...
## 16、列举出web项目中存在的安全问题以及解决方法 ##
1.    Sql注入：addslashes函数转义
Xss攻击：防xss工具htmlpurifier
总的来说，对于用户的输入要转义后在存入数据库
1.    登陆密码暴力破解
     对每个用户输错密码的次数做限制，比如输错3次以上当日不能登陆，或者加入验证码来防止暴力破解
      对于密码，如果有必要应使用多次md5加密并混入其他字符
1.    文件上传漏洞
上传文件要判断文件的类型和大小
1.    撞库
自动识别异常ip（1分钟尝试登陆100次），对于这些ip，整理成一个库并禁止访问
## 17、写出能够加快web端访问速度的方法 ##
先判断web段速度慢的原因，
...
## 18、提升数据库查询效率的方法除了使用缓存之外，还有哪些方法？ ##
...
echo -10%3;        -1
echo count(‘abc’)     1
echo date('Y-m-d H:i:s',strtotime('-2 day'));   前天这个时刻的时间


## 22、truncate和delete命令有何区别 ##
